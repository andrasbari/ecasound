...
protected:
    ...
    virtual string do_name(void) const {
        return ("Unit test for eca-sample-conversion");
    }
...
void ECA_SAMPLE_CONVERSION_TEST::do_run(void) {
    ...
    // max float about 0.99999999953
    double max = -((double) INT32_MAX / (double) INT32_MIN);
    if (cur < max || cur > max) {
...
#define S32INTFLOATINT(y) \
  { \
    ...
    if (verbose) cout << "s32 ifi " << (y) << " to " << mid << " to " << res << endl; \
    if (res != (y)) { \
